  ...
  <div class="container-fluid px-3 px-lg-4 py-4">
    ...
    <div class="row g-3 mt-1">
      <div class="col-12 col-xl-5">
        ...
        <article
          class="metric-card mb-3 <?= (!empty($subscription) && $subscription['subscription_status'] === 'active' && strtotime($subscription['subscription_ends_at']) > time()) ? 'metric-success' : 'metric-danger' ?>">
          <div class="metric-top">
            <span class="metric-label">Account Status</span>
            <span class="metric-icon">
              <i class="bi <?= (!empty($subscription) && $subscription['subscription_status'] === 'active' && strtotime($subscription['subscription_ends_at']) > time()) ? 'bi-shield-check' : 'bi-shield-exclamation' ?>"
                aria-hidden="true"></i>
            </span>
          </div>
          ...
          <div class="metric-meta">
            <?php if (!empty($subscription) && $subscription['subscription_status'] === 'active' && strtotime($subscription['subscription_ends_at']) > time()): ?>
              <span>Valid access until:
                <strong><?= date('d M Y', strtotime($subscription['subscription_ends_at'])) ?></strong></span>
            <?php else: ?>
        ...
        <div class="panel">
          <div class="panel-header border-bottom pb-3 mb-3">
            <div>
              <h2 class="h5 mb-1 section-title"><i class="bi bi-qr-code-scan" aria-hidden="true"></i><span>Renew via UPI
                  Payment</span></h2>
          ...
          <div class="p-3 bg-light border rounded text-center mb-4">
            <small class="text-muted text-uppercase fw-semibold d-block mb-2 small">Step 1: Send funds to Virtual
              Private Address (VPA)</small>

            <a href="upi://pay?pa=9584010740@ptsbi&pn=Radix%20Softwares&am=500.00&cu=INR"
              class="btn btn-primary btn-lg fw-bold px-4 py-2 my-1 shadow-sm">
              <i class="bi bi-phone-vibrate me-2"></i> Pay Now (₹500)
            </a>

            <div class="mt-2 text-muted small"><i class="bi bi-wallet2 me-1"></i> Supports GPay, PhonePe, Paytm, & BHIM
              UPI</div>
          </div>
          ...
          <form action="/admin/subscription/submit" method="POST">
            <?= csrf_field() ?>
            <div class="mb-4">
              <label for="transaction_id" class="form-label fw-semibold text-dark">Step 2: Enter Transaction
                Details</label>
              <p class="text-muted small mt-0 mb-2">Paste the 12-digit UPI Ref No, UTR Code, or Transaction Key from
                your payment receipt:</p>
              <div class="input-group">
                <span class="input-group-text bg-white"><i class="bi bi-key text-muted"></i></span>
                <input type="text" name="transaction_id" id="transaction_id"
                  class="form-control text-monospace text-uppercase" placeholder="e.g. 3145XXXXXXXX" required
                  autocomplete="off">
              </div>
            </div>
      ...
      <div class="col-12 col-xl-7">
        <div class="panel h-100">
          <div class="panel-header">
            <div>
              <h2 class="h5 mb-1 section-title"><i class="bi bi-clock-history" aria-hidden="true"></i><span>Transaction
                  Logs</span></h2>
                ...
                <?php if (!empty($payments)): ?>
                  <?php foreach ($payments as $p): ?>
                    <tr>
                      ...
                      <td>
                        <code
                          class="text-dark font-weight-bold small text-monospace"><?= esc($p['gateway_payment_id']) ?></code>
                      </td>
                      ...
                      <td class="text-end">
                        ...
                        <?php elseif ($p['payment_status'] === 'failed'): ?>
                          <span class="badge text-bg-danger px-2.5 py-1.5 shadow-sm">Failed</span>
                        <?php else: ?>
                          <span
                            class="badge text-bg-secondary px-2.5 py-1.5 shadow-sm"><?= strtoupper(esc($p['payment__status'])) ?></span>
                        <?php endif; ?>
                      </td>
                    </tr>
                  <?php endforeach; ?>
